...
void evolve_3pnt(float complex *psi, struct run_param *this_run, float dt)
{
  ...
  float rho = dt/SQR(this_run->delta_x);
  // diag and off-diag component of the matrix T
  float diag_imag = -2.0*this_run->hbar*rho/4.0;
  float off_diag_imag = this_run->hbar*rho/4.0;
  ...
  for(int32_t i=0;i<this_run->nmesh_x;i++) {
    gsl_vector_complex_set(rhs, i,
			   gsl_complex_rect(crealf(psi[i]), cimagf(psi[i])));
  }

  // (I-MT)*rhs = -MT*rhs + rhs
  gsl_blas_zgemv(CblasNoTrans,
		 gsl_complex_rect(-1.0, 0.0), MT, rhs,
  ...
  // copy the solution into the float complex array.
  for(int32_t i=0;i<this_run->nmesh_x;i++) {
