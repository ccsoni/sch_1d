...
void evolve_5pnt(double complex *psi, struct run_param *tr, double dt)
{
  ...
  if(first_call) {
    ...
    //    MT_ = gsl_matrix_complex_alloc(tr->nmesh_x, tr->nmesh_x);
    rhs_ = gsl_vector_complex_alloc(tr->nmesh_x);
    //    lhs = gsl_vector_complex_alloc(tr->nmesh_x);    
...
void evolve_3pnt(double complex *psi, struct run_param *tr, double dt)
{
  // (I-T)*\Psi^{n+1) = (I+T)*\Psi^n
  // (I+MT)*\Psi^(n+1) = (I-MT)*\Psi^n with MT=-T
