...
// time evolution of the schroedinger equation with 7pt approximation
// of the Laplace operator
void evolve_7pnt(double complex *psi, double *pot, struct run_param *tr,
		 double dt)
{
  ...
  double diag_imag =     -490.0*tr->hbar*rho/720.0;
  double off_diag1_imag = 270.0*tr->hbar*rho/720.0;
  double off_diag2_imag = -27.0*tr->hbar*rho/720.0;
  double off_diag3_imag =   2.0*tr->hbar*rho/720.0;
  ...
  if(first_call) {
    ...
    for(int32_t i=0;i<tr->nmesh_x;i++) {
      ...
      int32_t ip3=(i+3) % tr->nmesh_x;
      int32_t im3=(i-3+tr->nmesh_x) % tr->nmesh_x;
      gsl_matrix_complex_set(MT, i, ip3, gsl_complex_rect(0.0, -off_diag3_imag));
      gsl_matrix_complex_set(MT, i, im3, gsl_complex_rect(0.0, -off_diag3_imag));
    }
